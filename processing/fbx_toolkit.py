...
def export_tpose_npz(smplx_animation_path, betas_npz_path=None):
    ...
    """
    source_data = np.load(smplx_animation_path, allow_pickle=True)
    betas = source_data['betas']
    # Keep the source subject's gender so the SMPL-X add-on builds the T-pose
    # from the matching gendered base template, not the neutral one (which
    # lacks sex-specific body geometry and makes different bodies look alike).
    gender = str(source_data['gender']) if 'gender' in source_data else 'neutral'
...
def main():
    ...
    # Newer Blender versions install add-ons as namespaced extensions
    # (bl_ext.user_default.<name>) instead of the classic bare module name.
    for module_name in ('smplx_blender_addon', 'bl_ext.user_default.smplx_blender_addon'):
        try:
        ...
        raise RuntimeError("Could not enable the SMPL-X Blender add-on under any known module name.")
