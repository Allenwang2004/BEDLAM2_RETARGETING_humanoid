...
This repository provides tools to retarget human motion capture data onto humanoid characters — transferring motion
from a source human skeleton (e.g. SMPL-X) to target skeletons with different body proportions and morphologies.
...
For a concrete, reproducible run of this pipeline on AMASS data — including floor-height correction and video
rendering — see [docs/pipeline_walkthrough.md](docs/pipeline_walkthrough.md).
...
uv run make_fbx_files.py --input_dir <input_npz_dir> --output_dir <output_fbx_dir> -AMASS
```
