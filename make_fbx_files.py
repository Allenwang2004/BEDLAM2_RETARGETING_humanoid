...
def process_dir(input_dir, output_dir, processes, anim_format="SMPL-X", fps=30, tpose=False):
    ...
    for input_path in filenames:
        ...
        if input_path.parent == input_dir:
            # Flat layout: .npz sits directly in input_dir, no body_name/subj
            # nesting to derive a name from, so use the file's own stem.
            ...
            output_name = f"{body_name}_{body_name_suffix}"

        if tpose:
            output_path = output_dir / "bodies" / f"{body_name}.fbx"
        else:
            output_path = output_dir / "animations" / f"{output_name}.fbx"
