...
`pipeline_mujoco_cmu.md` documents the pipeline for one specific model
(`humanoid_CMU.xml`). This file is the checklist for repeating it with a
...
These steps only depend on MuJoCo's own body/geom arrays, not on
`humanoid_CMU`'s specific names:
...
1. Dump the new model's skeleton and inspect its body hierarchy:
   ```bash
   cd /Users/coconut/mujoco_humanoid_retargeting && source .venv/bin/activate
   python3 scripts/export_skeleton_json.py --input mjcf/<new_model>.xml \
