# End-to-End Pipeline: MuJoCo Humanoid (`humanoid_CMU.xml`) → Retargeted Video

This document records the full command sequence used to take a MuJoCo/
dm_control humanoid defined in an MJCF XML file (`humanoid_CMU.xml`),
build a matching Unreal Engine skeletal mesh for it, retarget AMASS/SMPL-X
motion onto it, and render the result locally. It complements
`pipeline_humantohuman.md` (SMPL-X → SMPL-X retargeting) — this pipeline
instead bridges a **non-SMPL, MuJoCo-native skeleton** into the same UE
retargeting machinery.

Two repos are involved:
- `/Users/coconut/mujoco_humanoid_retargeting` — MJCF, MuJoCo→JSON dump,
  Blender armature/FBX builder (no UE dependency).
- `/Users/coconut/bedlam2_retargeting` (this repo) — UE project, IK Rig/IK
...
| MuJoCo repo venv | `/Users/coconut/mujoco_humanoid_retargeting/.venv` (`mujoco`, `numpy`) |
| This repo | `retargeting/retargeting.uproject`, `paths.json` |

`paths.json` (this repo) holds `UNREAL_APP_PATH`, `BLENDER_APP_PATH`,
`PYTHON_SCRIPT_DIR` — the batch scripts (`retarget_batch.py`,
`import_batch.py`) read it via `handle_paths.py`. The commands below spell
out full paths directly since they invoke Blender/UE outside those batch
scripts.

## 1. Dump the MuJoCo rest-pose skeleton to JSON

Runs `mj_forward` at `qpos=0` and writes every body's world-space
position/orientation plus every geom's type/size/transform. Fully generic —
works on any MJCF humanoid, not just this one.

```bash
cd /Users/coconut/mujoco_humanoid_retargeting
source .venv/bin/activate
python3 scripts/export_skeleton_json.py \
  --input mjcf/humanoid_CMU.xml \
  --output assets/humanoid_skeleton.json
```

## 2. Build the Blender armature + mesh, export FBX

Builds a Blender armature (one bone per body, BFS from root; zero-offset
bodies like `lhipjoint`/`rhipjoint`/`lowerback` are merged into their
parent) plus capsule/sphere/ellipsoid placeholder meshes skinned 1:1 to
their body's bone, and exports it as FBX for UE import.

```bash
/Applications/Blender.app/Contents/MacOS/Blender --background --python \
  scripts/build_armature_fbx.py -- \
  --input assets/humanoid_skeleton.json \
  --output assets/humanoid_CMU.fbx
```
...
```bash
 /Applications/Blender.app/Contents/MacOS/Blender --background --python /Users/coconut/mujoco_humanoid_retargeting/scripts/extract_fbx_pose.py -- \
  --fbx /Users/coconut/bedlam2_retargeting/output/cmu_fbx/humanoid_CMU+Female_LiftBox_subj_Anim.fbx \
  --output /Users/coconut/mujoco_humanoid_retargeting/output/pose.json

 /Applications/Blender.app/Contents/MacOS/Blender --background --python /Users/coconut/mujoco_humanoid_retargeting/scripts/extract_fbx_pose.py -- \
  --fbx /Users/coconut/mujoco_humanoid_retargeting/assets/humanoid_CMU.fbx \
  --output /Users/coconut/mujoco_humanoid_retargeting/output/tpose.json
```
...
uv run scripts/fbx_pose_to_qpos.py \
  ...
  --pose-json /output/pose.json \
  --tpose-json /output/tpose.json \
  --output output/qpos/humanoid_CMU+Female_LiftBox_subj_qpos.npz
```
